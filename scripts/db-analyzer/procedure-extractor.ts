...
-- ================================================================`;
  ...
  /**
   * Procedure 본문 포맷팅 - CREATE PROCEDURE 문 완전한 형태로 생성
   *
   * 🔨 포맷팅 과정:
   * 1. 파라미터 문자열 생성: 모드, 이름, 데이터 타입 조합
   * 2. CREATE PROCEDURE 문 생성: 파라미터 리스트 포함
   * 3. BEGIN...END 블록: 전체 프로시저 본문
   * 4. 들여쓰기 적용: 가독성을 위한 4카 들여쓰기
   *
   * 💡 MySQL 호환성: 정확한 문법으로 오류 없이 실행 가능
   */
...
END`;
  ...
  /**
   * Function 본문 포맷팅 - CREATE FUNCTION 문 완전한 형태로 생성
   *
   * ⚙️ 포맷팅 과정:
   * 1. 파라미터 문자열 생성: 이름, 데이터 타입 조합
   * 2. CREATE FUNCTION 문 생성: 파라미터 리스트 및 반환 타입 포함
   * 3. 함수 속성: READS SQL DATA, DETERMINISTIC 설정
   * 4. BEGIN...END 블록: 전체 함수 본문
   * 5. 들여쓰기 적용: 가독성을 위한 4카 들여쓰기
   *
   * 💡 MySQL 호환성: 정확한 문법으로 오류 없이 실행 가능
   */
...
END`;
  ...
  /**
   * SQL 본문 들여쓰기 - 가독성 향상을 위한 코드 포맷팅
   *
   * 📝 들여쓰기 규칙:
   * 1. 4카 들여쓰기: 모든 라인에 4개 공백 추가
   * 2. 빈 라인 처리: 빈 라인은 그대로 유지
   * 3. 줄바꿈 보존: 원본 코드의 줄바꿈 구조 유지
   *
   * 💡 목적: 생성된 SQL 파일의 가독성 향상
   */
